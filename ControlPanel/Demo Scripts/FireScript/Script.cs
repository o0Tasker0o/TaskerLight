...
class TaskerLightScript
{
    ...
    */
    public static Color[] TickLighting(long elapsedMS)
    {
        ...
        {
            float hue = (float)(rand.NextDouble() / 9.0f) + 0.025f;
            float sat = 1.0f - ((float)rand.NextDouble() * 0.125f);
            float value = (float)rand.NextDouble() / 2.0f;
            value += 0.25f;
            mLEDColors[ledIndex] = HSVToRGB(hue, sat, value);
        }
